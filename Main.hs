...
import Pacman.World
import Pacman.Level
import qualified Pacman.Engine.World as Engine
...
main = do
    ...
    inputPacmanDirectionRef <- newIORef inputPacmanDirection
...
update worldRef inputRef = do
    ...
    modifyIORef worldRef (\_ -> Engine.worldUpdate dt input world)
